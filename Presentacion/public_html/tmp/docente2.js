...
// Model for Profesor entity
    
    // Collection class for Profesor entities
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ProfesorCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-profesor-list-itemis template identifier for item
                        templateName: '#tpl-profesor-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.profesor = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.profesor,
                    // tpl-profesor-details is template identifier for chosen model element
                    templateName: '#tpl-profesor-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Profesor();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-profesor-details is a template identifier for chosen model element
                templateName: '#tpl-profesor-details',

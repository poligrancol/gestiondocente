...
// Model for ArchivosAdjuntos entity
    models.ArchivosAdjuntos = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/Logica/webresources/edu.poli.prap.gd.data.archivosadjuntos/",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('idArchivo');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/Logica/webresources/edu.poli.prap.gd.data.archivosadjuntos/';
            }
    ...
    // Collection class for ArchivosAdjuntos entities
    models.ArchivosAdjuntosCollection = Backbone.Collection.extend({
        model: models.ArchivosAdjuntos,
        url: "http://localhost:8080/Logica/webresources/edu.poli.prap.gd.data.archivosadjuntos/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ArchivosAdjuntosCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-archivosadjuntos-list-itemis template identifier for item
                        templateName: '#tpl-archivosadjuntos-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.archivosadjuntos = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.archivosadjuntos,
                    // tpl-archivosadjuntos-details is template identifier for chosen model element
                    templateName: '#tpl-archivosadjuntos-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.ArchivosAdjuntos();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-archivosadjuntos-details is a template identifier for chosen model element
                templateName: '#tpl-archivosadjuntos-details',
